...
def get_openai_client() -> Optional[Any]:
    """Create an OpenAI client using Streamlit secrets or env vars.

    Supports:
      - OpenAI SDK v1+ (preferred): from openai import OpenAI
      - Legacy OpenAI SDK 0.x (fallback): import openai
    """
    api_key = get_secret("openai_api_key") or get_secret("OPENAI_API_KEY")
    if not api_key:
        st.warning("OpenAI API key not found. Calendar parsing is disabled.")
        return None
    ...
    st.warning("OpenAI SDK not available. Calendar parsing is disabled.")
    return None

def _openai_chat_completions_create(client: Any, **kwargs) -> Any:
    """Compatibility wrapper for OpenAI SDK v1+ and legacy 0.x."""
    # New SDK (v1+)
    ...
    raise RuntimeError("Unsupported OpenAI client object")

def _openai_get_message_content(resp: Any) -> str:
    """Extract assistant message content from both SDK response shapes."""
    # New SDK response (object)
    try:
...
def parse_slots_from_image(image: Image.Image) -> List[Dict[str, str]]:
    ...
    try:
        resp = _openai_chat_completions_create(client, 
            model="gpt-4o-mini",
            temperature=0,
            messages=[
        ...
        content = (_openai_get_message_content(resp) or "").strip()
